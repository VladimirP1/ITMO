     ...
     otherwise -> 
      do
        ...
        let sourceStmt = if axiomN >= 0 || contextN >= 0 
            then [] 
            else (filter (\x -> Map.member x stmtToInputLine) (causeToList $ lookupFailOver xBS resultToCause (CauseMB [])))
...
printAnnotatedMapped lineNumber mapping (bs, ann) 
    | (Axiom i) <- (ann) = putStrLn ("[" ++ (show $ mapz lineNumber) ++ ". Ax. sch. " ++ (show i) ++ "] " ++ (fromByteString bs))
    | (Assumption i) <- (ann) = putStrLn ("[" ++ (show $ mapz lineNumber) ++ ". Hypothesis " ++ (show i) ++ "] " ++ (fromByteString bs))
    | (ModusPonens i j) <- (ann) = putStrLn ("[" ++ (show $ mapz lineNumber) ++ ". M.P. " ++ (show $ mapz j) ++ ", " ++ (show $ mapz i) ++ "] " ++ (fromByteString bs))
